...
#
# USAGE:
#   bash memory-manual.sh [spec-folder] [description]
#
# EXAMPLES:
#   bash memory-manual.sh 001-skills-and-hooks "Bug fix session"
#   bash memory-manual.sh 008-cli-devtools-routing
#   bash memory-manual.sh  # Auto-detect most recent folder
#
# PURPOSE:
#   Provides easy manual invocation of memory when hooks are
...
# Parse arguments
SPEC_FOLDER="${1:-}"
DESCRIPTION="${2:-Manual memory invocation}"
...
ARGUMENTS:
  ...
  description   Optional. Brief description of session
                Default: "Manual memory invocation"
...
EXAMPLES:
  ...
  bash memory-manual.sh

  # Save with custom description
  bash memory-manual.sh 008-cli-devtools-routing "Anchor implementation"
...
TEMP_JSON=$(mktemp "/tmp/memory-manual-XXXXXX.json")
...
jq -n \
  ...
  '{
    SPEC_FOLDER: $spec,
    recent_context: [{
      request: "Manual memory invocation",
      completed: $desc,
