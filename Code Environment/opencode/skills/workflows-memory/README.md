...
cd .claude/skills/workflows-memory && npm install
...
node .claude/skills/workflows-memory/scripts/generate-context.js \
  /tmp/context-data.json "122-feature-name"

# Helper script (auto-detects folder)
bash .claude/skills/workflows-memory/scripts/memory-manual.sh
```
...
```bash
cd .claude/skills/workflows-memory
...
# Test embedding
node -e "
require('.claude/skills/workflows-memory/scripts/lib/embeddings')
...
node .claude/skills/workflows-memory/scripts/index-all.js --scan .
```
...
Both platforms use the same backend implementation in `.claude/skills/workflows-memory/`. The `.opencode/` folder contains synced copies for Opencode's skill discovery, but the actual scripts run from `.claude/`.
