#!/usr/bin/env bats
# ───────────────────────────────────────────────────────────────
# Tests for: verify-spec-compliance.sh
...
HOOK_SCRIPT="$REAL_HOOKS_DIR/PostToolUse/verify-spec-compliance.sh"
...
@test "verify-spec-compliance processes Write tool" {
  local input
...
@test "verify-spec-compliance processes Edit tool" {
  local input
...
@test "verify-spec-compliance ignores Read tool" {
  local input
...
@test "verify-spec-compliance ignores Bash tool" {
  local input
...
# ───────────────────────────────────────────────────────────────
# SPEC FOLDER AWARENESS
# ───────────────────────────────────────────────────────────────

@test "verify-spec-compliance warns without spec folder" {
  local input
  input=$(make_write_input "/project/src/file.js")

  run bash -c "echo '$input' | bash '$HOOK_SCRIPT'"

  # Should warn about missing spec folder
  [ "$status" -eq 0 ]
}

@test "verify-spec-compliance respects skip marker" {
  # Create skip marker would need real file system setup
  local input
...
@test "verify-spec-compliance handles empty input" {
  run bash -c "echo '{}' | bash '$HOOK_SCRIPT' 2>/dev/null"

  [ "$status" -eq 0 ]
}

@test "verify-spec-compliance handles missing file_path" {
  local input
...
@test "verify-spec-compliance always exits 0" {
  local input
...
@test "verify-spec-compliance never blocks" {
  local input
...
@test "verify-spec-compliance completes within 200ms" {
