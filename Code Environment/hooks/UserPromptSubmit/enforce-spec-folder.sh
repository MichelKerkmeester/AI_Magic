...
# Early definition needed for multi-stage flow handler
...
#
# Stage 1 (spec_folder): User answered spec folder question
#   → If folder has memory files, emit MEMORY_LOAD question
...
if [ "$CHECK_SPEC_FOLDER" != "false" ]; then
  ...
  if [ -n "$SPEC_FOLDER" ] && has_substantial_content "$SPEC_FOLDER"; then
    ...
    if present_memory_selection_prompt "$SPEC_FOLDER"; then
      ...
      echo "🧠 [enforce-spec-folder] Memory files found, presented selection prompt" >&2
    else
      echo "✅ [enforce-spec-folder] No memory files found, proceeding without context" >&2
    fi
