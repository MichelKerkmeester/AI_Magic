...
- [7. 🎯 COMMANDS (5 TOTAL)](#7--commands-5-total)
...
└── references/
    ├── automation_workflows.md           # Workflow enforcement, context auto-save
...
Six automation scripts in `.opencode/speckit/scripts/` provide workflow automation.
...
**Example**:
```bash
$ .opencode/speckit/scripts/recommend-level.sh --loc 250 --files 8
...
**Example**:
```bash
$ .opencode/speckit/scripts/archive-spec.sh specs/042-user-auth
...
Five SpecKit commands available in OpenCode.
...
- Location: `.opencode/command/speckit_*.md` (and `.opencode/command/speckit_*.md` mirror)
...
| `workflows-memory` | Saves to spec folder memory/         |
...
A: Context is saved manually using `/save_context` or by asking the AI to save context.
