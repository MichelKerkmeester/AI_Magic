...
mkdir -p "/Users/YOUR_NAME/projects/MCP Servers"
cd "/Users/YOUR_NAME/projects/MCP Servers"
```
...
```json
{
  "mcpServers": {
    "mnemo": {
      ...
      "args": [
        "run",
        "/Users/YOUR_NAME/projects/MCP Servers/mnemo/packages/local/src/stdio.ts"
      ],
      "env": {
        "GEMINI_API_KEY": "your-gemini-api-key-here",
        "MNEMO_PORT": "8080",
        "MNEMO_DIR": ".opencode/mnemo"
      },
      "disabled": false
    }
  }
...
| `MNEMO_DIR` | Data directory (local only) | .opencode/mnemo |
| `MNEMO_AUTH_TOKEN` | Auth token for protected endpoints | None |

**Note on MNEMO_DIR**: Use `.opencode/mnemo` instead of `~/.mnemo` because the `~` character doesn't expand in JSON environment variables. Project-local storage (`.opencode/mnemo`) keeps cache data organized with other OpenCode assets and ensures proper path resolution.
