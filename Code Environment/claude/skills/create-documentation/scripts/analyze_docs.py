...
def extract_code_snippets(content: str) -> List[CodeSnippet]:
    ...
    while i < len(lines):
        ...
        if line.strip().startswith('```'):
            ...
            # Get context (collect up to 5 previous non-empty lines, use last 3)
            context_lines = []
            for j in range(max(0, i-5), i):
                if lines[j].strip():
                    context_lines.append(lines[j].strip())
            context = ' '.join(context_lines[-3:])  # Use last 3 lines for context
...
def analyze_snippet(snippet: CodeSnippet) -> List[str]:
    ...
    if len(lines) > 3:
        list_markers = sum(1 for line in lines if re.match(r'^\s*[-*]\s|\d+\.\s', line))
...
def print_report(analysis: Dict):
    ...
    if analysis['duplicate_pairs']:
        ...
        for i, (idx1, idx2) in enumerate(analysis['duplicate_pairs'][:5], 1):
            print(f"{i}. Duplicate code snippets found at indices {idx1} and {idx2}")
