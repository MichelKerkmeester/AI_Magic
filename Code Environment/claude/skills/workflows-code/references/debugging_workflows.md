...
**When to use**: ANY frontend technical issue (console errors, layout bugs, animation issues, event handler failures, video/media problems, performance issues, cross-browser inconsistencies)
...
```json
{
  ...
  "url": "https://example-project.com/video-player.js",
  "lineNumber": 45,
  "columnNumber": 12,
...
```javascript
// Flame graph shows yellow bar >50ms in processData()
...
modal.addEventListener('click', handleClick, { once: true });
```
...
function processData() {
  const largeData = fetchLargeDataset();  // 10MB array

  document.querySelector('.btn').addEventListener('click', () => {
    console.log('Clicked');  // Closure holds reference to largeData
  });
}
...
function processData() {
  ...
  document.querySelector('.btn').addEventListener('click', () => {
    console.log('Clicked');  // No reference to largeData
  });
}
```
...
### Pairs With
- **implementation_workflows** - Debug timing and validation issues
- **verification_workflows** - Verify fixes work correctly
- **shared_patterns** - Use common DevTools and logging patterns

### Browser Tools Used
- Chrome DevTools MCP (automated debugging via MCP tools)
- Chrome DevTools (manual inspection when needed)
- Device Emulation (viewport simulation via `resize_page`)
- Network Monitor (via `list_network_requests`)
- Performance Profiler (via `performance_start_trace`)
