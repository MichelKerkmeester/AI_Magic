...
> - workflows-memory SKILL.md - Workflow details (in your project's `.opencode/skills/` or `.opencode/skills/`)
...
```
.opencode/memory/database/memory-index.sqlite
```
...
mkdir -p "/Users/YOUR_NAME/Projects/mcp-servers/semantic-memory"
cd "/Users/YOUR_NAME/Projects/mcp-servers/semantic-memory"
```
...
cp /path/to/.opencode/memory/scripts/semantic-memory.js .

# Copy lib directory
mkdir -p lib
cp /path/to/.opencode/memory/scripts/lib/embeddings.js lib/
cp /path/to/.opencode/memory/scripts/lib/vector-index.js lib/
cp /path/to/.opencode/memory/scripts/lib/trigger-matcher.js lib/
cp /path/to/.opencode/memory/scripts/lib/trigger-extractor.js lib/
cp /path/to/.opencode/memory/scripts/lib/retry-manager.js lib/
```
...
ln -s "/Users/YOUR_NAME/Projects/mcp-servers/semantic-memory/node_modules" .
```
...
```json
{
  "mcpServers": {
    "semantic_memory": {
      "command": "node",
      "args": [
        "/Users/YOUR_NAME/Projects/mcp-servers/semantic-memory/semantic-memory.js"
      ],
      "env": {},
      "disabled": false
    }
...
The default database path is `.opencode/memory/database/memory-index.sqlite`. This can be overridden via environment variable:
...
```bash
sqlite3 .opencode/memory/database/memory-index.sqlite "SELECT 'OK: ' || COUNT(*) || ' memories' FROM memory_index" 2>/dev/null || echo "Database not created yet (will be created on first save)"
```
...
| Skills SKILL.md | Your project's `.opencode/skills/` or `.opencode/skills/` | Memory workflow     |
