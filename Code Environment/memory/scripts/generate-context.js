...
const CONFIG = {
  ...
  TEMPLATE_DIR: path.join(__dirname, '..', 'templates'),  // .opencode/memory/templates/
  // Fix: Derive PROJECT_ROOT from script location, not cwd()
  // __dirname = .opencode/memory/scripts
  // PROJECT_ROOT = 3 levels up from __dirname
...
async function detectSpecFolder(collectedData = null) {
  ...
  if (sessionId) {
    markerPaths.push(path.join(CONFIG.PROJECT_ROOT, '.opencode', `.spec-active.${sessionId}`));
  }
  markerPaths.push(path.join(CONFIG.PROJECT_ROOT, '.opencode', '.spec-active'));
...
async function collectSessionData(collectedData, specFolderName = null) {
  ...
  const OBSERVATIONS_DETAILED = observations.map(obs => {
    ...
    return {
      TYPE: detectObservationType(obs).toUpperCase(),
