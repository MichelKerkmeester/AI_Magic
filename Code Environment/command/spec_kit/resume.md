...
elif [ -f ".opencode/.spec-active" ]; then
  SPEC_FOLDER=$(cat .opencode/.spec-active)
else
  # Priority 2: Find most recent memory file
  RECENT_MEMORY=$(find specs -path "*/memory/*.md" -type f 2>/dev/null | \
    xargs ls -t 2>/dev/null | head -1)
  if [ -z "$RECENT_MEMORY" ]; then
    echo "No memory files found"
    SPEC_FOLDER=""
  else
    SPEC_FOLDER=$(echo "$RECENT_MEMORY" | sed 's|/memory/.*||')
  fi
fi
...
### Step 6: Load & Execute Workflow Prompt

Based on execution mode (`:auto` suffix or user choice):

- **AUTONOMOUS**: Load and execute `.opencode/command/spec_kit/assets/spec_kit_resume_auto.yaml`
- **INTERACTIVE**: Load and execute `.opencode/command/spec_kit/assets/spec_kit_resume_confirm.yaml`
...
- **Option C**: List all available memory files and let user select specific ones
...
Ready to continue. What would you like to do?
  ...
  B) Run /spec_kit:implement to execute remaining tasks
  C) Just chat - I'll work on it myself
```
