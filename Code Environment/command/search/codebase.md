...
Present options to user based on current state:
...
**If "Search codebase" selected:** Ask the user for query:
```yaml
...
**If "Index git history" selected:** Ask the user for count:
```yaml
...
- If triggered by menu: ask the user for query
