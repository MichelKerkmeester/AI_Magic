  ...
  SKILL:
    ...
    field_formats:
      ...
      allowed-tools:
        ...
        description: "List of Claude Code tools"
...
│
└─> README
    ├─ Has skill-style frontmatter? → Valid if in .claude/skills/
    └─ No frontmatter? → Valid (no action needed)
```
...
field_inference:
  ...
  allowed_tools:
    ...
    python: |
      tool_pattern = r'\b(Read|Write|Edit|Bash|Grep|Glob|WebFetch|AskUserQuestion)\('
