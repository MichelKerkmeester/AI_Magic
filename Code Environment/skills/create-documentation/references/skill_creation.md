...
Comprehensive reference for creating effective skills that extend Claude's capabilities with specialized knowledge, workflows, tool integrations, and bundled resources.
...
This reference provides complete guidance for creating, validating, and distributing Claude skills. It covers the full lifecycle from initial concept through packaging and maintenance.
...
- Same code being rewritten repeatedly by Claude
...
scripts/init_skill.py markdown-optimizer --path .claude/skills
...
- Symptom: Claude misinterprets skill guidance
...
- Symptom: Claude recreates same code/content repeatedly
...
- Symptom: Claude doesn't use skill when appropriate
...
**Command**:
```bash
markdown-document-specialist --validate .claude/skills/my-skill/SKILL.md
```
...
**Problem**: Claude recreates same code repeatedly instead of using scripts.
...
**Problem**: Skill exists but never triggers because conditions unclear.
...
# Good
## 1. WHEN TO USE
Automatic enforcement via hooks after Write/Edit operations.
...
3. Skill automatically available to Claude

**Verification**:
- Check skill appears in Claude's skill list
...
## 13. 🔄 ASSET FILE MIGRATION GUIDE

### Purpose

This guide helps convert existing asset files from markdown prose to structured logic representations (Python/YAML pseudo-code) for improved clarity and AI-friendliness.

**Target files**: Asset files in `.claude/skills/*/assets/*.md` with logic/conditional content

**Benefits**:
- **Clarity**: Structured logic is easier to understand than prose
- **AI-Friendly**: LLMs parse Python/YAML better than narrative descriptions
- **Maintainability**: Logic changes are more visible
- **Consistency**: Same patterns across all asset files


### When to Use Each Logic Format
...
- Pipeline definitions (multi-step workflows)
...
Scan asset files for these patterns (candidates for conversion):

```markdown
# Markdown prose patterns to convert:

## Pattern 1: Decision Logic in Lists
- If condition A, do action X
- If condition B, do action Y
- Otherwise, do action Z

## Pattern 2: Workflow Steps in Lists
...
## Pattern 4: Conditional Prose
"The system should check X. If Y is true, then Z. Otherwise, W."
```

**Step 2: Choose Conversion Format**

Based on complexity:
- **Simple (2-5 branches)** → Markdown decision tree
- **Medium (workflow/config)** → YAML structure
- **Complex (algorithms)** → Python pseudo-code

**Step 3: Convert Content**

See examples below for before/after patterns.

**Step 4: Validate**

- Semantic equivalence (meaning preserved)
- Syntax highlighting works (```python, ```yaml blocks)
- Markdown renders correctly


### Conversion Examples

#### Example 1: Mode Detection Logic
...
#### Example 2: Frontmatter Requirements
...
├─> Command        → MUST have frontmatter (description, argument-hint)
...
document_creation_workflow:
  phase_1_analysis:
    ...
    validation: "Type must be one of: SKILL, Command, Knowledge, Spec"
  ...
  phase_6_quality_check:
    action: "Run c7score and quality metrics"
...
**Before migrating an asset file**:

```markdown
Analysis:
□ File contains logic/conditional prose
□ File has >50 lines of content
□ Logic complexity justifies structured format
□ Conversion improves clarity

Conversion:
□ Identified appropriate format (Python/YAML/decision tree)
□ Converted logic while preserving semantics
□ Added syntax highlighting (```python or ```yaml)
□ Kept explanatory prose as markdown

Documentation:
□ Added ToC if file >100 lines
□ Updated section numbering if needed
□ Added examples showing usage
□ Cross-referenced from SKILL.md if needed

Validation:
□ Semantic equivalence verified (no meaning lost)
□ Syntax highlighting works
□ ToC links work (if added)
□ Markdown renders correctly
...
After migrating asset files, update templates to show the new patterns:

**Files to update**:
1. `skill_asset_template.md` - Add logic representation sections ✅ (Completed)
2. `skill_reference_template.md` - Add workflow examples ✅ (Completed)
3. `skill_creation.md` - Add migration guide ✅ (This section)

**Reference implementations**:
- See `.claude/skills/z_example/` for target patterns
- See `.claude/skills/create-documentation/assets/skill_asset_template.md` for updated template
- See `.claude/skills/create-documentation/assets/skill_reference_template.md` for workflow examples

### Common Pitfalls

**❌ Don't**:
- Convert ALL prose to code (keep explanatory text)
- Break semantic equivalence (meaning must be preserved)
- Add complexity without benefit (simple lists may stay as lists)
- Forget syntax highlighting (```python, ```yaml required)
- Skip validation (always verify rendering)

**✅ Do**:
- Convert logic/workflows that benefit from structure
- Preserve semantic meaning exactly
- Add ToC for files >100 lines
- Use appropriate format for complexity level
- Validate rendering in multiple viewers
