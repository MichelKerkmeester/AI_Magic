...
Comprehensive reference for execution modes, validation patterns, enforcement workflows, and phase interactions for the markdown documentation quality pipeline.
...
Workflows define the execution patterns and operational modes for the markdown documentation quality pipeline. These workflows orchestrate enforcement, optimization, and validation phases.
...
| **Enforcement Only** | 1 | Manual or `--enforce` | File save, structure validation | Valid markdown |
...
- Saving files → Enforcement (manual or via `--enforce`)
...
# (Run post-write validation manually to fix filename if needed)
...
| "Style dimension N/A" | No style guide | Create `document_style_guide.md` in project root |
