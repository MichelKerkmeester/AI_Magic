...
Guide the creation of effective OpenCode skills through a structured 6-step workflow: Understanding → Planning → Initialization → Editing → Packaging → Iteration.
...
def route_documentation_resources(task):
    ...
    if task.mode == "document_quality":
        ...
        # ──────────────────────────────────────────────────────────────────
        # Workflows
        # Purpose: Four execution modes, enforcement patterns
    ...
    if task.mode == "skill_creation":
        ...
        # ──────────────────────────────────────────────────────────────────
        # Command Template
        # Purpose: Slash command templates (simple, workflow, mode-based, destructive)
...
# references/workflows.md → Four execution modes, enforcement patterns
...
# assets/command_template.md → Slash command templates
...
5. **ALWAYS ask about llms.txt generation** (prompt user for consent, never auto-generate)
...
1. **Document type ambiguous** (ask the user for clarification)
...
**User prompts**: Required for llms.txt generation consent, document type clarification, major restructuring approval
...
- `code_standards.md` - Code example validation (project-specific)
  - If missing: Skip code validation
  - Used by: Code example formatting checks

**Recommended** (create if needed):
- `conversation_documentation.md` - Spec folder system documentation
...
markdown-document-specialist --full-pipeline .opencode/skills/my-skill/SKILL.md
```
