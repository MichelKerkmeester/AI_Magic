...
description: Mandatory spec folder workflow orchestrating documentation level selection (1-3), template selection, and folder creation for all file modifications through documentation enforcement and context auto-save.
allowed-tools: ["*"]
version: 1.0.2
---
...
3. AI archives existing files to `001-{topic}/` (numbered)
4. AI creates new sub-folder with user-provided name
5. AI copies fresh templates to new sub-folder
6. `.spec-active` marker updated to point to new sub-folder
7. Each sub-folder maintains independent `memory/` directory
...
```
1. AI detects existing root-level content in spec folder
2. AI displays: "📦 SUB-FOLDER VERSIONING WILL BE APPLIED"
3. User selects Option A
4. AI asks: "Please provide a name for the new sub-folder (e.g., 'api-refactor')"
5. User provides: "spec-folder-versioning"
6. AI creates archive and new sub-folder structure:
...
When returning to an active spec folder, the AI agent asks **two separate questions in sequence**:
...
1. AI detects active spec folder
