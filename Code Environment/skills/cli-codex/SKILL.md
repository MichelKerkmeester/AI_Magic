...
allowed-tools: [Bash, Read, Write, Grep, Glob, mcp__semantic_memory__memory_search]
version: 2.0.0
---
...
Enable AI agents to effectively orchestrate OpenAI Codex CLI (v0.58.0+) with GPT-5.1 Codex for code generation, review, analysis, and specialized tasks requiring a second AI perspective.
...
- Context is already loaded and understood by the AI agent
...
TASK CONTEXT
    ...
    │   └─► ACTIVATE this skill
    │
    └─► Standard AI agent task (no second opinion needed)
...
**Spec Folder Awareness**: Codex CLI (v0.58.0+) is trained to respect documentation requirements. It may ask for spec folders before generating code.
...
# 2. Review (can use AI agent or Codex)
...
# [Use AI agent's native capabilities]
...
6. **NEVER use when context preservation is critical**
   - Codex exec doesn't see the agent's conversation history
   - Each invocation is isolated
   - Handle directly when context matters
...
4. **ESCALATE IF task requires conversation history**
   - Codex can't access the agent's context
...
- Pattern: Agent implements → Codex reviews → Fix issues → Test
...
- `code_standards.md` - Project-specific code standards to validate Codex's generated code
