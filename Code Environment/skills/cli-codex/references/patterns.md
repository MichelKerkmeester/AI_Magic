...
# Option B: AI agent reviews Codex's work
# [Use AI agent's native review capabilities]
...
**Pattern**: Use the AI agent and Codex to validate each other's work.
...
# [AI agent creates implementation]

# Codex reviews AI agent's code
codex exec "Review [ai-agent-generated-file] for: 1) correctness, 2) security, 3) performance, 4) edge cases" 2>&1
...
# AI agent reviews Codex's code
# [AI agent performs review]
```
...
# Continue with other work using AI agent
# [AI agent continues with implementation]
...
- **Don't**: Use Codex for simple tasks AI agent handles well
...
### Pattern: AI Agent Implements, Codex Validates

```bash
# Phase 1: AI agent implements feature
# [AI agent generates implementation]

# Phase 2: Codex reviews
codex exec "Review [ai-agent-implementation] for: security, performance, edge cases" 2>&1

# Phase 3: Address findings
# [AI agent or Codex fixes issues]

# Phase 4: Final validation
# [Run tests, manual review]
```

### Pattern: Codex Generates, AI Agent Integrates

```bash
# Phase 1: Codex generates component
codex exec "Generate [component] with [specs]" --full-auto 2>&1

# Phase 2: AI agent integrates into codebase
# [AI agent handles imports, dependencies, testing, integration]
...
```bash
# Stream 1: AI agent works on feature A
# [AI agent implements feature A]
...
# [AI agent coordinates integration]
```
...
if [ $EXIT_CODE -ne 0 ]; then
  ...
  if grep -q "Rate limit" output.log; then
    ...
    echo "Unknown error. Falling back to AI agent."
    # Use AI agent instead
  fi
fi
```
...
# If Codex fails or produces poor output, use AI agent
if [ $? -ne 0 ] || [ ! -s codex_output.log ]; then
echo "Codex failed. Using AI agent instead."
    # [Use AI agent for task]
fi
```
