...
The memory system supports **3 independent execution paths**. Any method can be used standalone without dependencies on the others.
...
| **Helper Script** | No                | Automation, batch operations       | Low    | 0          |
...
├────────────────────────────────────────────────────────────────┤
│                                                                │
│   Slash Command          Direct Script         Helper Script   │
│   ┌──────────┐          ┌──────────┐          ┌──────────┐     │
│   │ /memory/ │          │  node    │          │   bash   │     │
│   │  save    │          │ script.js│          │ manual.sh│     │
│   └────┬─────┘          └────┬─────┘          └────┬─────┘     │
│        │                     │                     │           │
│        ▼                     ▼                     ▼           │
│   ┌──────────┐          ┌──────────┐          ┌──────────┐     │
│   │ AI Agent │          │   JSON   │          │   Auto   │     │
│   │ Analysis │          │   Input  │          │  Detect  │     │
│   └────┬─────┘          └────┬─────┘          └────┬─────┘     │
│        │                     │                     │           │
│        └─────────────────────┴─────────────────────┘           │
│                              │                                 │
...
└─────────────────────────────────────────────────────────────────┘
              ...
              YES                           NO
               ...
               ▼                             ▼
┌─────────────────────────┐   ┌─────────────────────────┐
│ Do you need automatic   │   │ Do you have JSON data   │
│ conversation analysis?  │   │ prepared already?       │
└────────────┬────────────┘   └────────────┬────────────┘
             │                             │
      ┌──────┴──────┐               ┌──────┴──────┐
      │             │               │             │
     YES           NO              YES           NO
      ...
      ▼             ▼               ▼             ▼
┌───────────┐ ┌───────────┐   ┌───────────┐ ┌───────────┐
│   SLASH   │ │  DIRECT   │   │  DIRECT   │ │  HELPER   │
│  COMMAND  │ │  SCRIPT   │   │  SCRIPT   │ │  SCRIPT   │
│           │ │ (custom   │   │           │ │ (auto-    │
│ /memory/  │ │  JSON)    │   │ node ...  │ │  detect)  │
│   save    │ │           │   │           │ │           │
└───────────┘ └───────────┘   └───────────┘ └───────────┘
```
...
| Quick manual save, no AI available        | **Helper Script**  |
| Testing memory system functionality       | **Direct Script**  |
| Batch processing multiple saves           | **Helper Script**  |
...
| AI agent active  | Check response capability      | Use Helper Script instead |
| Spec folder set  | `cat .spec-active` or detect   | Specify folder manually   |
...
| Script exists      | `test -f .opencode/memory/scripts/generate-context.js` | Check skill installation |
...
**When to Use:** Manual saves without AI agent, automation workflows
**Requirement:** Node.js and jq installed

**Location:** `.opencode/memory/scripts/memory-manual.sh`

### Usage Patterns

```bash
# Save to specific spec folder with description
...
# Show help
bash .opencode/memory/scripts/memory-manual.sh --help
```

### Features

- **Auto-detection:** Finds most recent spec folder if not specified
- **Minimal input:** Only requires spec folder name
- **JSON generation:** Creates proper JSON structure automatically
- **Error handling:** Validates inputs before execution

### Validation Checkpoints

| Checkpoint        | Verification                                        | Action on Failure        |
| ----------------- | --------------------------------------------------- | ------------------------ |
| jq installed      | `which jq`                                          | Install jq               |
| Script exists     | `test -f .opencode/memory/scripts/memory-manual.sh` | Check skill installation |
| Script executable | `test -x ...`                                       | `chmod +x` script        |
| Spec folder valid | Directory exists                                    | Check folder name        |

---

## 6. 🏷️ ANCHOR RETRIEVAL
...
# Smart search with context script
.opencode/scripts/load-related-context.sh "###-spec" smart "your keywords"

# Extract specific anchor directly
...
### Context Retrieval Commands Reference

| Command         | Purpose                       | Example                                                        | Token Cost |
| --------------- | ----------------------------- | -------------------------------------------------------------- | ---------- |
| `list`          | Show all memory sessions      | `load-related-context.sh "049-..." list`                       | N/A        |
| `summary`       | Load summary from recent file | `load-related-context.sh "049-..." summary`                    | ~400       |
| `search <kw>`   | Find anchors by keyword       | `load-related-context.sh "049-..." search "oauth"`             | N/A        |
| `extract <id>`  | Load specific section         | `load-related-context.sh "049-..." extract "decision-jwt-049"` | ~600       |
| `recent N`      | Load last N summaries         | `load-related-context.sh "049-..." recent 3`                   | ~900       |
| `smart <query>` | Relevance-ranked search       | `load-related-context.sh "049-..." smart "auth bug"`           | ~600       |
...
| Script available     | `test -f .opencode/scripts/load-related-context.sh` | Use grep/sed directly      |
...
node .opencode/memory/scripts/generate-context.js --help
```

---

*Related: [SKILL.md](../SKILL.md) | [troubleshooting.md](./troubleshooting.md) | [anchor_format.md](./anchor_format.md)*
