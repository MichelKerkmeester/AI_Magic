...
element.textContent = userInput;  // Safe, no HTML rendering

// ❌ BAD: innerHTML for plain text
element.innerHTML = userInput;  // Unnecessary risk
```
...
const userEmail = input.value;  // No validation
sendToAPI(userEmail);
```
...
const allowedOrigins = ['https://anobel.com'];
...
exec(userCommand);  // Command injection vulnerability!
```

**Never Execute User Input:**
```javascript
// ❌ BAD: eval() with user input
eval(userInput);  // Extreme vulnerability

// ❌ BAD: Function constructor
new Function(userInput)();  // Also vulnerable
...
localStorage.setItem('password', userPassword);  // Vulnerable to XSS
localStorage.setItem('token', authToken);  // Accessible to all scripts
```
...
**Apply during:**
- **Phase 1 (Implementation)**: Use checklist when writing validation code
- **Phase 2 (Debugging)**: Check for security vulnerabilities if issues arise
- **Phase 3 (Verification)**: Verify security headers and input validation
