...
**See**: .opencode/skills/workflows-chrome-devtools/SKILL.md for complete CLI automation workflows
...
### Pairs With
- **implementation_workflows** - Verify implementations work correctly
- **debugging_workflows** - Verify fixes work after debugging
- **shared_patterns** - Use standard DevTools verification patterns

### Browser Tools Used
- Chrome DevTools MCP (automated testing via MCP tools)
- Chrome DevTools (manual inspection when needed)
- Device Emulation (viewport simulation: 375px, 768px, 1920px)
- Network Monitor (via `list_network_requests`)
- Performance Profiler (via `performance_start_trace`/`performance_stop_trace`)
