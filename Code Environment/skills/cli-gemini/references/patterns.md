...
Advanced patterns for orchestrating Gemini CLI effectively from AI agents with explicit reasoning and validation checkpoints.

---

## 1. 🔄 CORE PATTERNS

Fundamental patterns for reliable Gemini CLI orchestration.
...
**Purpose**: Ensure high-quality code generation through self-review

**Actions**:
1. Generate code with Gemini
2. Have Gemini review its own work
3. Fix identified issues

**Checkpoint questions**:
- Have security vulnerabilities been identified?
- Has Gemini reviewed its own output?
- Are all identified issues addressed?
...
Patterns for optimizing resource usage and workflow efficiency.
...
Sophisticated patterns for complex workflows and quality assurance.
...
### Pattern 9: Cross-Validation with Primary Agent

Use both AIs for highest quality.

**Primary Agent Generates, Gemini Reviews:**
```bash
# 1. Agent writes code (using normal agent tools)
# 2. Gemini reviews
gemini "Review this code for bugs and security issues: [paste code]" -o text
```

**Gemini Generates, Primary Agent Reviews:**
```bash
# 1. Gemini generates
gemini "Create [code]" --yolo -o text

# 2. Agent reviews the output (in conversation)
# "Review this code that Gemini generated..."
```

**Different Perspectives:**
- Primary agent: Strong on reasoning, following complex instructions
...
| ----------------------- | ----------------------------------- | ------------------------------------ |
| Complex code generation | Generate-Review-Fix                 | Self-correction catches mistakes     |
| Long-running tasks      | Background Execution                | Continue other work while processing |
| Rate limit management   | Model Selection Strategy            | Different models, different quotas   |
| High-quality output     | Cross-Validation with Primary Agent | Two AI perspectives                  |
| Multi-step workflows    | Incremental Refinement              | Easier to debug, clear audit trail   |
| Current web information | Context Enrichment + Web Search     | Grounded in latest data              |
| Unknown codebase        | Codebase Investigator + Analysis    | Deep architectural understanding     |
